...
import { ProcyonSerial, DeviceInfo, OneSecondData, TestResult } from '@/lib/usb/procyon';

interface DeviceContextType {
  device: ProcyonSerial | null;
  connected: boolean;
  connecting: boolean;
...
export function DeviceProvider({ children }: { children: ReactNode }) {
  const [device, setDevice] = useState<ProcyonSerial | null>(null);
  ...
  const connect = useCallback(async (baudRate?: number) => {
    ...
    try {
      const serial = new ProcyonSerial(baudRate);
      ...
      const info = await serial.getDeviceInfo();
      setDeviceInfo(info);
    } catch (err) {
